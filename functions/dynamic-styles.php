...
/* ------------------------------------ */
if ( ! function_exists( 'squareread_hex2rgb' ) ) {

	function squareread_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'squareread_enqueue_google_fonts' ) ) {

	function squareread_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'squareread_enqueue_google_fonts' );


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'squareread_dynamic_css' ) ) {

	function squareread_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = squareread_hex2rgb($color_1);
			...
			// container width
			if ( get_theme_mod('container-width','1440') != '1440' ) {			
...
#nav-header-nav.nav-menu:not(.mobile) .menu ul { background: '.esc_attr( get_theme_mod('color-1') ).'; }
#nav-header-nav.nav-menu:not(.mobile) .menu ul:after { border-bottom-color: '.esc_attr( get_theme_mod('color-1') ).'; }
#nav-header-nav.nav-menu:not(.mobile) .menu ul ul:after { border-right-color: '.esc_attr( get_theme_mod('color-1') ).'; border-bottom-color: transparent; }
...
.wp-pagenavi span.current { border-bottom-color: '.esc_attr( get_theme_mod('color-1') ).'!important; }				
			...
			// mobile menu color
			if ( get_theme_mod('color-mobile-menu','#333333') != '#333333' ) {
				$styles .= '
#wrap-nav-mobile .search-expand,
#nav-mobile-nav.nav-menu.mobile { background-color: '.esc_attr( get_theme_mod('color-mobile-menu') ).'; }	
				'."\n";
			}
			// topbar menu color
			if ( get_theme_mod('color-topbar-menu','#333333') != '#333333' ) {
				$styles .= '
#wrap-nav-topbar .search-expand,
#wrap-nav-topbar { background-color: '.esc_attr( get_theme_mod('color-topbar-menu') ).'; }
#nav-topbar-nav.nav-menu:not(.mobile) .menu ul { background: '.esc_attr( get_theme_mod('color-topbar-menu') ).'; }
#nav-topbar-nav.nav-menu:not(.mobile) .menu ul:after { border-bottom-color: '.esc_attr( get_theme_mod('color-topbar-menu') ).'; }
#nav-topbar-nav.nav-menu:not(.mobile) .menu ul ul:after { border-right-color: '.esc_attr( get_theme_mod('color-topbar-menu') ).'; border-bottom-color: transparent; }

				'."\n";
			}
			// logo background
			if ( get_theme_mod('color-logo','') != '' ) {
				$styles .= '.site-title { background-color: '.esc_attr( get_theme_mod('color-logo') ).'; }'."\n";
			}			
			// header color
			if ( get_theme_mod('color-header','#222222') != '#222222' ) {
				$styles .= '#header { background-color: '.esc_attr( get_theme_mod('color-header') ).'; }'."\n";
			}
...
#footer-bottom #back-to-top { background-color: '.esc_attr( get_theme_mod('color-footer-menu') ).'; }
			...
			// footer color
			if ( get_theme_mod('color-footer','#333333') != '#333333' ) {
			...
			wp_add_inline_style( 'squareread-style', $styles );	
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'squareread_dynamic_css' );
